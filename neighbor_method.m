...
%% Performing the SSVEP detection algorithm
% Preparing data
for subject = 1:35    
    filename =[ '~\Benchmark dataset\S' num2str(subject) '.mat'];
    load(filename); % filterdata 64 channels * 1500 points * 40 targets * 6 trials(blocks)
    
    eeg = filterdata(channel_selection,len_delay_smpl+1:len_delay_smpl+samples,:,:);
    ...
    for loocv_i =  1:1:num_blocks
        ...
        trains = zeros(num_targs,num_fbs,num_chans,samples);
        W = zeros(num_fbs,num_targs,num_chans); 

        for targs_i = 1:num_targs
            template = squeeze(mean(train_all(:,:,targs_i,:),4)); 

            potential_stimuli = [targs_i-num_cols,targs_i-1,targs_i,...
                                 targs_i+1,targs_i+num_cols];
        ...
        for tw_i = 1:length(time_windows)
            ...
            for targs_i = 1:num_targs                
                test = eeg(:,1:len_gaze_smpl,targs_i,testtrial); 
                
                r = zeros(num_fbs,num_targs);
                for fb_i = 1:num_fbs
                    testdata = filterbank(test, fs, fb_i);                   
                    for targs_j = 1:num_targs
